...
const TrainingPage: React.FC = () => {
  ...
  const fetchMaterials = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/training/materials?folderId=${selectedFolderId}`);
      const data = await response.json();
      setMaterials(data);
    } catch (error) {
  ...
  return (
    <Box display="flex" height="100vh">
      <Box width="300px" p={2} borderRight="1px solid #ccc">
        <Typography variant="h6" style={{color: 'black'}}>Folders</Typography>
        <Button
          variant="contained"
          color="primary"
      ...
      <Box flex={1} p={2}>
        {selectedFolderId ? (
          <>
            <Typography variant="h6" style={{color: 'black'}}>Materials in Folder</Typography>
            <UploadForm folderId={selectedFolderId} onUploadSuccess={fetchMaterials} />
            <Divider sx={{ marginY: 2 }} />
            <MaterialList materials={materials} />
          </>
        ) : (
          <Typography variant="h6" style={{color: 'black'}}>Select a folder to view materials</Typography>
        )}
      </Box>
      <CreateFolderDialog
